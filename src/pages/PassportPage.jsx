  ...
  async function processAudioWithAI(blob) {
    ...
    reader.onloadend = async () => {
      try {
        ...
        // Support both platform-injected key and standard Vite/Vercel prefixed key
...
- вернуть отредактированный структурированный текст (до 500 символов), который можно сразу вставлять в отчет о визите.
...
В торговой точке присутствует наша продукция (матрасы). Продавец сообщил, что последние поставки были давно. Необходимо отправить обновленный прайс-лист.`,
        ...
        alert("Помилка обробки аудіо. Перевірте налаштування AI.")
      } finally {
                ...
                <div className="glass-soft rounded-3xl p-4 space-y-3">
                  ...
                  <div className="space-y-1 pt-1">
                    {manufacturers.selected.map((it, idx) => {
                      const label =
                        manufacturersList.find((m) => m.id === it.manufacturerId)?.label ?? it.manufacturerId
                      const ppP = pct(clampInt(it.pp, 0, 9999), totalPP)
                      const kvP = pct(clampInt(it.kv, 0, 9999), totalKV)
